<?php if (!defined('BASEPATH')) exit('No direct script access allowed');
  ...
  function forinput($list) {
    ...
    switch ($list['input_type']) {
      ...
      break;
      case 'N':
        $set_sp = $list['sparator_num'] == 'Y'?'number_separator':'numeric';
        $hasil = '<input class="form-control '.$set_sp.'" '.$mixl.' type="number" name="'.$list['key_to_param'].'" id="'.$list['key_to_param'].'" placeholder="'.$list['fieldnm'].'">';
      break;
      case 'T':
