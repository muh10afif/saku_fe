    ...
    <div class="row align-items-center">
        ...
        <div class="col-sm-6">
            <ol class="breadcrumb float-right">
                ...
                <li class="breadcrumb-item">Master Home</li>
...
<div class="row">
    ...
    <div class="col-md-7">
        <div class="card shadow ">
            <div class="card-body table-responsive">
                <table class="table table-bordered table-hover" style="border-collapse: collapse; border-spacing: 0; width: 100%;" id="tabel_temukan_kami" width="100%" cellspacing="0">
    ...
    <div class="col-md-5">
        <div class="card shadow">
            ...
            <form id="form_temukan_kami" autocomplete="off">
                <input type="hidden" name="id_temukan_kami" id="id_temukan_kami">
                <input type="hidden" name="aksi" id="aksi" value="Tambah">
                <div class="card-body d-flex justify-content-center">
                    <div class="col-md-12">
                         
                        <div class="form-group">
                            <label for="nama" class="col-form-label text-left">Nama<span class="text-danger">*</span></label>
                            <div class="">
                                <input type="text" class="form-control" id="nama" name="nama" placeholder="Masukkan Nama" required data-parsley-required-message="Deskripsi harus terisi.">
                            </div>
                        </div>  
                        <div class="form-group">
                            <label for="isi" class="col-form-label text-left">Isi<span class="text-danger">*</span></label>
                            <div class="">
                                <input type="text" class="form-control" id="isi" name="isi" placeholder="Masukkan Isi" required data-parsley-required-message="Deskripsi harus terisi.">
                            </div>
                        </div>  
                        ...
                        <div class="form-group mt-2">
                            <label for="asuransi" class="col-form-label text-left">Sosmed</label><br>
                            <button type="button" class="btn btn-toggle st_sosmed" data-toggle="button" aria-pressed="false" autocomplete="off" value="1">
                                
                            <div class="handle"></div>
                            </button>
                        </div>

                        <input type="hidden" id="val_sosmed" name="sosmed" value="1">
                        ...
                        <div class="form-group text-center mt-1 mb-0">
                            ...
                            <button type="button" class="btn btn-danger mt-1 batal_temukan_kami" id="batal_temukan_kami"><i class="fas fa-times mr-1"></i> Batal</button>
                        </div>   
...
<script>
    ...
    $(document).ready(function () {
        ...
        var tabel_list_temukan_kami = $('#tabel_temukan_kami').DataTable({
            "processing"        : true,
            "serverSide"        : true,
            "order"             : [],
            "ajax"              : {
                "url"   : "<?= base_url() ?>Temukan_kami/tampil_data_temukan_kami",
                "type"  : "POST",
                "data"  : function (data) {
                    data.read               = "<?= $role['read'] ?>";
        ...
        // 06-09-2021
        $('#form_temukan_kami').parsley({
            errorsContainer: function(el) {
                return el.$element.closest('.form2');
            }
        });

        $('#batal_temukan_kami').on('click', function () {
            reset_form();
        })

        function reset_form() {
            $('#aksi').val('Tambah');
            $('#form_temukan_kami').trigger("reset");
            $('#form_temukan_kami').parsley().reset();

            $('#kode_temukan_kami').val('');
            $('#temukan_kami').val('');
            $('#image').val('');
            $("#ImgPreview").attr("src", "");
            $('.btn-rmv1').removeClass('rmv');
            ...
            $('.st_sosmed').attr('aria-pressed', 'false');
            $('.st_sosmed').attr('value', '1');
            $('.st_sosmed').removeClass('active');
        ...
        $('#form_temukan_kami').on('submit', function () {

            var form_temukan_kami = new FormData(this);
            ...
            swal({
                ...
                if (result.value) {
                    $.ajax({
                        url     : "<?= base_url() ?>Temukan_kami/simpan_temukan_kami",
                        type    : "POST",
                        beforeSend  : function () {
                            swal({
                                title   : 'Menunggu',
                        ...
                        success : function (data) {
                            ...
                            $('.kode_temukan_kami').val(data.kode_temukan_kami);

                            tabel_list_temukan_kami.ajax.reload(null,false);        
                            reset_form();
        ...
        $('#tabel_temukan_kami').on('click', '.edit', function () {

            var id_temukan_kami     = $(this).data('id');
            var nama                = $(this).attr('nama');       
            var isi                 = $(this).attr('isi');       
            var sosmed              = $(this).attr('sosmed');       
            var image               = $(this).attr('image');

            $('#aksi').val('Ubah');
            $('.judul').text('Ubah Data');
            $('#id_temukan_kami').val(id_temukan_kami);
            $('#nama').val(nama);
            $('#isi').val(isi);
            ...
            if (sosmed == '1') {
                ...
                $('.st_sosmed').attr('aria-pressed', 'true');
                $('.st_sosmed').attr('value', '0');
                $('.st_sosmed').addClass('active');
            }
        ...
        $('#tabel_temukan_kami').on('click', '.hapus', function () {
            ...
            swal({
                    ...
                    if (result.value) {
                        $.ajax({
                            ...
                            data        : {aksi:'Hapus', id_temukan_kami:id_temukan_kami},
                            dataType    : "JSON",
                            success     : function (data) {
