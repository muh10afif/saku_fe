...
<div class="row">
    ...
    <div class="col-md-7">
        <div class="card shadow ">
            <div class="card-body table-responsive">
                <table class="table table-bordered table-hover" style="border-collapse: collapse; border-spacing: 0; width: 100%;" id="tabel_lob" width="100%" cellspacing="0">
                    <thead class="thead-light text-center">
                        <tr>
                            <th width="5%">No</th>
                            <th width="20%">Kode LOB</th>
                            <th width="20%">LOB</th>
    ...
    <div class="col-md-5">
        <div class="card shadow">
            ...
            <form id="form_lob" autocomplete="off">
                <input type="hidden" name="id_lob" id="id_lob">
                <input type="hidden" name="aksi" id="aksi" value="Tambah">
                <div class="card-body d-flex justify-content-center">
                    <div class="col-md-12">
                        
                        <div class="form-group">
                            <label for="kode_lob" class="col-form-label text-left">Kode LOB</label>
                            <div class="">
                                <input type="hidden" class="form-control kode_lob" id="kode_lob_asli" name="kode_lob_asli" value="<?= $kode_lob ?>">
                                <input type="text" class="form-control kode_lob" id="kode_lob" name="kode_lob" value="<?= $kode_lob ?>" readonly>
                            </div>
                        </div>  
                        <div class="form-group">
                            <label for="lob" class="col-form-label text-left">LOB<span class="text-danger">*</span></label>
                            <div class="">
                                <input type="text" class="form-control" id="lob" name="lob" placeholder="Masukkan COB" required data-parsley-required-message="COB harus terisi.">
                            </div>
                        </div>  
                        <div class="form-group">
                            <label for="deskripsi" class="col-form-label text-left">Deskripsi<span class="text-danger">*</span></label>
                            <div class="">
                                <textarea name="deskripsi" id="deskripsi" rows="5" class="form-control" placeholder="Masukkan Deskripsi" required data-parsley-required-message="Deskripsi harus terisi."></textarea>
                            </div>
                        </div>  
                        ...
                        <div class="form-group text-center mt-1 mb-0">
                            ...
                            <button type="button" class="btn btn-danger mt-1 batal_lob" id="batal_lob"><i class="fas fa-times mr-1"></i> Batal</button>
                        </div>   
...
<script>
    ...
    $(document).ready(function () {
        ...
        var tabel_list_lob = $('#tabel_lob').DataTable({
            "processing"        : true,
            "serverSide"        : true,
            "order"             : [],
            "ajax"              : {
                "url"   : "<?= base_url() ?>Lob/tampil_data_lob",
                "type"  : "POST",
                "data"  : function (data) {
                    data.read               = "<?= $role['read'] ?>";
        ...
        $('#batal_lob').on('click', function () {
            reset_form();
        })

        function reset_form() {
            $('#aksi').val('Tambah');
            $('#form_lob').trigger("reset");
            $('#form_lob').parsley().reset();
            ...
            var kode_lob = $('#kode_lob_asli').val();
            $('#kode_lob').val(kode_lob);
        ...
        $('#form_lob').on('submit', function () {

            var form_lob = new FormData(this);
                ...
                if (result.value) {
                    $.ajax({
                        ...
                        success : function (data) {
                            ...
                            $('.kode_lob').val(data.kode_lob);

                            tabel_list_lob.ajax.reload(null,false);        
                            reset_form();
        ...
        $('#tabel_lob').on('click', '.edit', function () {

            var id_lob      = $(this).data('id');
            var kode_lob    = $(this).attr('kode_lob');       
        ...
        $('#tabel_lob').on('click', '.hapus', function () {
